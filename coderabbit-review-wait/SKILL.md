...
| | Codex (retired) | Copilot (retired) | CodeRabbit |
|---|---|---|---|
| Clean pass | `+1` reaction | review of head with zero inline comments | review of head with no inline findings, no `CHANGES_REQUESTED` and no collapsed section (with `request_changes_workflow: true` that review is an **`APPROVED`** one) |
