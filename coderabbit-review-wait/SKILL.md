...
|---|---|---|---|
| Clean pass | `+1` reaction | review of head with zero inline comments | review of head with no inline findings, no `CHANGES_REQUESTED` and no collapsed section (with `request_changes_workflow: true` that review is an **`APPROVED`** one), and nothing undispositioned from a second reviewer (see `RESULT=OTHERBOT`) |
...
|---|---|---|
| `RESULT=CLEAN` | A review at head, no inline findings, no collapsed section, no `CHANGES_REQUESTED`, and nothing undispositioned from another reviewer | Gate met. Merge. |
