...
> **Dormant since 2026-09-17.** The Codex subscription is paused, so `chatgpt-codex-connector` no longer reviews PRs. The active PR review gate is **`copilot-review-wait`** (`copilot-pull-request-reviewer[bot]`), whose clean pass is a review of the head commit with **zero inline comments** — Copilot has no `+1`. Everything below still applies verbatim if the subscription resumes; the triage, divergence and reply sections are reviewer-agnostic and are mirrored in the Copilot skill.
