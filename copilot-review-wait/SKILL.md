...
description: "[ACTIVE for the five repositories Copilot reviews: tokscale-core, NyanCogs, pilotfish-grok, homebrew-tokenbar, Syrtis-Agent. Everything at or above ten stars is reviewed by CodeRabbit - use coderabbit-review-wait there. The split exists because CodeRabbit's OSS tier makes a public repository under ten stars trigger its reviews by hand, so those five moved to Copilot on 2026-09-19 and their copilot_code_review rulesets are active again.] Wait for the GitHub Copilot PR reviewer (copilot-pull-request-reviewer[bot]) to finish reviewing a pull request, then classify the outcome. A clean pass requires all of: a review at the head commit, a body matching a VERIFIED clean shape ('### Approval recommended' with 'Comments generated: 0' and a complete 'Files reviewed: N/N' ratio), zero inline comments, no suppressed section, and nothing undispositioned from another reviewer. Any other body shape is UNREAD, not clean - including a review object with no status line at all, which is what Copilot submits when its quota is exhausted, and that quota is per requesting user so it takes out every repository on this side of the split at once. Findings belonging to a reviewer this gate does not read are OTHERBOT, because a second reviewer can be triggered onto any pull request by hand. Before starting, asks the user to pick a mode - auto loop (fix, reply, resolve threads, push, and keep tracking until clean or budget) or a single wait-and-report run. Use right after opening a non-draft PR, marking one ready, or pushing to a PR branch in a repo whose ruleset auto-requests Copilot review - each of those triggers one review round, so poll without asking again. In a repo without that ruleset, request the review first (the script's --request flag). Drafts are never reviewed, so do not poll one. Optionally takes a PR number; otherwise it uses the current branch's PR and auto-detects the repo. Codex is retired."
---
...
|---|---|---|
| `tokscale-core`, `NyanCogs`, `pilotfish-grok`, `homebrew-tokenbar`, `Syrtis-Agent` — all under 10 stars | **GitHub Copilot** | this one |
| `sepia`, `pilotfish`, `coralline`, `TokenBar`, `remora-cc`, `Syrtis-Windows`, `calico-claude` — 10 stars and up | **CodeRabbit** | `coderabbit-review-wait` |

The split is a constraint, not a preference. CodeRabbit's OSS tier states: *"For public repositories with less than 10 stars, CodeRabbit requires reviews to be triggered manually."* Automatic CodeRabbit review on those five therefore ends with the Advanced trial on 2026-10-02, so they moved to Copilot on 2026-09-19: their `copilot_code_review` rulesets are active again with `review_on_push: true`, and each sets `reviews.auto_review.enabled: false` in its `.coderabbit.yaml` so two reviewers do not both run during the remaining trial days.

Check before assuming which applies — `gh api repos/OWNER/NAME -q .stargazers_count`. A repository crossing ten stars can move back, and `tokscale-core` at 3 stars is the one to watch: it is the shared Rust engine behind TokenBar and Syrtis-Windows, so it carries more risk than its star count suggests.

An on-demand `@coderabbitai review` still works on all five.
...
| `RESULT=OTHERBOT count=N` | Copilot is clean, but N undispositioned findings on this PR belong to a reviewer this gate does not read — an unresolved thread, or a finding in that reviewer's own review body, which creates no thread at all. **A clean verdict here means COPILOT found nothing, not that the PR is clean.** Open the PR, read that reviewer's output, disposition each finding. |
...
| `RESULT=OTHERBOT count=N` | Copilot is clean, but N undispositioned findings belong to a reviewer this gate does not read — unresolved threads, or findings in that reviewer's review body, which create no thread | **Not a pass.** Open the PR and read that reviewer's output. Measured on `NyanCogs#29`: CodeRabbit auto-review disabled there, triggered by hand anyway, 8 inline findings, three on lines Copilot never touched. |
...
| `RESULT=NOT_REVIEWED` | A review object at head whose body is not a verdict — no `### <status>` line. Usually the quota message, printed in full above the result line | **Not a pass, and not a timeout either.** No review happened. The Copilot quota is per requesting user, so when it is exhausted every repository on the Copilot side of the routing table has no automatic reviewer at the same moment. Wait for the reset, or trigger the other reviewer by hand with a top-level `@coderabbitai review` comment — which works on exactly these repositories because their CodeRabbit auto review is disabled and the manual command is the documented escape hatch. |
